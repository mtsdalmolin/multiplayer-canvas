...
function cmd(program: string, args: string[] = []) {
    ...
    p.stdout.on('data', (data: unknown) => process.stdout.write(data));
    // @ts-ignore [stderr may be null?]
    p.stderr.on('data', (data: unknown) => process.stderr.write(data));
...
cmd('http-server', ['-p', '3000', '-a', '127.0.0.1', '-s', '-c-1', '-d', 'false'])
